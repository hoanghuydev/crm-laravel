    ...
    <div class="bg-white shadow sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            ...
            <form method="POST" action="{{ route('customer-types.update', $customerType) }}" class="space-y-6">
                ...
                <input type="hidden" name="is_active" value="{{ $customerType->is_active ? 1 : 0 }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" id="minimum_score" name="minimum_score" 
                                   value="{{ old('minimum_score', isset($customerType->minimum_score) ? $customerType->minimum_score : 0) }}" 
                        ...
                        <div>
                            ...
                            <input type="number" id="priority" name="priority" 
                                   value="{{ old('priority', isset($customerType->priority) ? $customerType->priority : 1) }}" 
                ...
                @php
                    $weights = $customerType->scoring_weights ?? [];
                    $hasCustomWeights = !empty($weights);
                @endphp
                <div class="border-t border-gray-200 pt-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <button type="button" id="toggle-weights" class="text-sm text-blue-600 hover:text-blue-500">
                            {{ $hasCustomWeights ? 'Use Default Weights' : 'Use Custom Weights' }}
                        </button>
                    </div>
                    <p class="text-sm text-gray-500 mb-4">Leave empty to use default weights. If specified, all weights must sum to 1.0</p>
                    
                    <div id="scoring-weights" class="grid grid-cols-1 md:grid-cols-2 gap-4 {{ $hasCustomWeights ? '' : 'hidden' }}">
                        <!-- Total Value Weight -->
                        <div>
                            ...
                            <input type="number" id="total_value_weight" name="scoring_weights[total_value_weight]" 
                                   value="{{ old('scoring_weights.total_value_weight', $weights['total_value_weight'] ?? '') }}" 
                        ...
                        <div>
                            ...
                            <input type="number" id="order_count_weight" name="scoring_weights[order_count_weight]" 
                                   value="{{ old('scoring_weights.order_count_weight', $weights['order_count_weight'] ?? '') }}" 
                        ...
                        <div>
                            ...
                            <input type="number" id="order_frequency_weight" name="scoring_weights[order_frequency_weight]" 
                                   value="{{ old('scoring_weights.order_frequency_weight', $weights['order_frequency_weight'] ?? '') }}" 
                                   min="0" max="1" step="0.01" placeholder="0.25"
                                   class="mt-1 block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <input type="number" id="location_weight" name="scoring_weights[location_weight]" 
                                   value="{{ old('scoring_weights.location_weight', $weights['location_weight'] ?? '') }}" 
                    ...
                    <div id="weights-sum" class="mt-2 text-sm text-gray-600 {{ $hasCustomWeights ? '' : 'hidden' }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let weightsVisible = !scoringWeights.classList.contains('hidden');
